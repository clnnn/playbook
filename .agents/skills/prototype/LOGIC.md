...
One file, plain HTML/CSS/JS — no framework, no bundler, no server, everything inline so it opens by double-click and survives being emailed around. [`PUBLISHING.md`](PUBLISHING.md) holds that page contract and the delivery path: hand over a live link where you can, so each new action or scenario reaches the recipient by refreshing the tab they already have open.
...
Keep it beautiful but restrained: clean typography, generous spacing, one accent colour. Nothing competes with the state and the buttons, so the motion and imagery [CRAFT.md](CRAFT.md) allows a product surface stay out of this one; what it says about states, words, and mocked data applies in full — a demo whose data reads as `foo` / `100` costs the domain expert the very fluency you brought them in for.

### 4. Hand it over

Send them the link, or open it for them. They'll click through the walkthroughs and free-play whenever they get to it; the interesting moments are when they say "wait, that shouldn't be possible" or "huh, I assumed X would be different" — those are the bugs in the _idea_, which is the whole point. If they want new actions or a new scenario, add them. Prototypes evolve.

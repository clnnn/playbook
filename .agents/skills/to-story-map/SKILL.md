---
name: to-story-map
description: Facilitated story mapping on a live whiteboard, adversarially reviewed, ending in one GitHub issue per release slice.
...
Facilitate a Jeff Patton story mapping session in which the user watches the map grow on a **live whiteboard**, put the finished map in front of two adversaries, then cut one GitHub issue per release slice.
...
Read the domain section of `AGENTS.md` and follow the pointer it names — the context map, each context's `CONTEXT.yaml`, the glossary. Name activities, steps, and tasks in the glossary's words, honouring its `avoid` synonyms, so the stories written from this map inherit the domain's vocabulary. An empty map, a missing `CONTEXT.yaml`, no glossary: proceed silently.
...
Then check whether this initiative is already mapped: `gh issue list --label map:<slug> --state all --json number,title,body`. Issues found take the resume branch. Nothing found opens at Q1.
...
`gh auth status` runs here, at the first step that needs GitHub. Unauthenticated stops at this gate with the map approved and the board live: hand the user the exact `gh auth login` command and hold, so nothing before this point is lost.

Read the initiative's existing issues (`--label map:<slug>`, matched with `--label release:rN`). An open slice issue is shown as a diff against the approved slice, with edit-in-place offered. A closed slice issue is reported and skipped: its stories are already written beneath it, and editing it would strand them.
...
Dispatch one subagent with the approved map, the write plan, and [`references/SLICE-FORMAT.md`](references/SLICE-FORMAT.md). One agent, working in order, because R2's body cites R1's issue number:

1. **Labels** — create the missing ones: `map:<slug>`, `release:r1`, `release:r2`, `release:r3`.
2. **Issues** — `gh issue create` per slice, R1 first, each body written per SLICE-FORMAT.md, each carrying `map:<slug>` plus its release label.
3. **Chain** — for each blocked slice, fetch the blocker's node id (`gh issue view <n> --json id`) and create the edge: `gh api repos/{owner}/{repo}/issues/{n}/dependencies/blocked_by -f issue_id=<node-id>`. R2 is blocked by R1, R3 by R2. An endpoint the repo does not support falls back to the issue body's `## Dependencies` prose as the single render, stated plainly; any other failure is reported as the error it is.

**Done when** three issues exist, each labelled, and every chain edge is created or its fallback stated.

### Report

The three issue URLs with their labels and blockers, the board's link, and the next steps: `/to-user-stories <slice-issue>` to break a slice into stories, `/prototype <r1-issue>` to build the walking skeleton.

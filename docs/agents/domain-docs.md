...
If these files don't exist, **proceed silently**. Don't flag their absence; don't suggest creating them upfront. The `domain-discovery` skill creates them lazily, as terms and boundaries actually get resolved.
...
Field-level formats live in `.agents/skills/domain-discovery/references/` — `CONTEXT-MAP-FORMAT.md` and `CONTEXT-FORMAT.md`.
...
If the concept you need isn't in the glossary yet, that's a signal — either you're inventing language the project doesn't use (reconsider) or there's a real gap (note it for `domain-discovery`).
...
One line, then carry on; the user decides. Rewriting the map belongs to `domain-discovery`, not to the skill that hit the conflict.

...
If any of these files don't exist, **proceed silently**. Don't flag their absence; don't suggest creating them upfront. The `grill-and-map` skill creates them lazily when terms, boundaries or decisions actually get resolved.
...
Field-level formats live in `.agents/skills/grill-and-map/references/` — `CONTEXT-MAP-FORMAT.md`, `CONTEXT-FORMAT.md` and `ADR-FORMAT.md`.
